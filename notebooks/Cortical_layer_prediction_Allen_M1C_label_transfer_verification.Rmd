---
title: "Verification of label transfer in M1C ALLEN DATA"
...
```{r tidy=TRUE}
con_m1c <- read_rds("../ctx_layers_SCZ_ALLEN/rds_objects/m1c_allen_only.con.rds")

annotations_m1c_high <- read_rds("../ctx_layers_SCZ_ALLEN/rds_objects/m1c_high_transf_annotations.rds")
annotations_m1c_med <- read_rds("../ctx_layers_SCZ_ALLEN/rds_objects/m1c_mid_transf_annotations.rds")
```



```{r}
annotations_m1c_med$labels %>% gsub("Other", "TESTTEST", .) %>% .[. == "TESTTEST"]
```
...
con_m1c$plotGraph(alpha = 0.4, groups = annotations_m1c_high$labels,
                  font.size = 4, mark.groups = T, size = 0.6,
                  show.legend = T, legend.pos = "bottom", 
#                  plot.na = F, keep.limits = T
#                  color.range = "data"
                  ) + scale_color_manual(values = palette_45[-(1:4)])
```

```{r, fig.height = 11, fig.width = 12, message=F}
con_m1c$plotGraph(alpha = 0.4, groups = annotations_m1c_high$labels[annotations_m1c_high$labels == "SST_NPY"],
                  font.size = 0, mark.groups = T, size = 0.6,
                  show.legend = T, legend.pos = "bottom", 
                  plot.na = F, keep.limits = T
#                  color.range = "data"
...
## Mid res
```{r, fig.height = 11, fig.width = 12, message=F}
con_m1c$plotGraph(alpha = 0.4, groups = annotations_m1c_med$labels,
                  font.size = 4, mark.groups = T, size = 0.6,
                  show.legend = T, legend.pos = "bottom"
#                  color.range = "data"
...
plot.list.scz.ast <- lapply(X = c("GFAP", "AQP4", "SLC1A3", "SLC1A2", "GJB6", "GLUL", "FGFR3", 
  ...
  con_m1c$plotGraph(gene = x, title = x, size = 0.3, alpha = 1)
}
)
...
plot.list.scz.olig <- lapply(X = c("MBP", "MOG", "MAG", "SOX10", "CNDP1", "OPALIN","MOBP"), function(x) {
  con_m1c$plotGraph(gene = x, title = x)
}
)
...
**ISSUE: OPCs are picked as some neurons: need to exclude those from analysis and map!!!!!!!**
...
**Issue: some microglia is picked as some neurons**
...
plot.list.scz.neur.exc <- lapply(X = c("CUX2", "LAMP5", "PDGFD", "MARCH1", "FREM3", "SV2C", "THEMIS", "SEMA3A",
                               ...
                               function(x) {
  con_m1c$plotGraph(gene = x, title = x, alpha= 1, size = 0.3
                    )
}
)
