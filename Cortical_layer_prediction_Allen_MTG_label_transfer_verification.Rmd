---
title: "Verification of label transfer in MTG ALLEN DATA"
...
```{r tidy=TRUE}
#con_mtg <- read_rds("../ctx_layers_SCZ_ALLEN/rds_objects/mtg_allen_only.con.rds")

#annotations_mtg_high <- read_rds("../ctx_layers_SCZ_ALLEN/rds_objects/mtg_high_transf_annotations.rds")
#annotations_mtg_med <- read_rds("../ctx_layers_SCZ_ALLEN/rds_objects/mtg_med_transf_annotations.rds")
```
...
## High res
```{r, fig.height = 14, fig.width = 7, message=F}
(con_mtg$plotGraph(alpha = 0.4, groups = annotations_mtg_high$labels[annotations_mtg_high$labels == "L5_FEZF2_ADRA1A"],
...
(con_mtg$plotGraph(alpha = 0.4, groups = annotations_mtg_high$labels,
                  font.size = 4, mark.groups = T, size = 0.6,
                  show.legend = T, legend.pos = "bottom", 
#                  plot.na = F, keep.limits = T
#                  color.range = "data"
...
con_mtg$plotGraph(alpha = 0.4, groups = annotations_mtg_med$labels,
                  font.size = 4, mark.groups = T, size = 0.6,
                  show.legend = T, legend.pos = "bottom"
#                  color.range = "data"
                  ) + scale_color_manual(values = palette_45)
...
plot.list.scz.ast <- lapply(X = c("GFAP", "AQP4", "SLC1A3", "SLC1A2", "GJB6", "GLUL", "FGFR3", 
  ...
  con_mtg$plotGraph(gene = x, title = x, size = 0.3, alpha = 1)
}
)
...
plot.list.scz.olig <- lapply(X = c("MBP", "MOG", "MAG", "SOX10", "CNDP1", "OPALIN","MOBP"), function(x) {
  con_mtg$plotGraph(gene = x, title = x)
}
)
...
plot.list.scz.neur.exc <- lapply(X = c("CUX2", "LAMP5", "PDGFD", "MARCH1", "FREM3", "SV2C", "THEMIS", "SEMA3A",
                               ...
                               function(x) {
  con_mtg$plotGraph(gene = x, title = x, alpha= 1, size = 0.3
                    )
}
)
